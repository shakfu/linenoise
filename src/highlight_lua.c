/* highlight_lua.c -- Tree-sitter based Lua syntax highlighting
 *
...
/* Simplified Lua highlighting query.
...
/* Map capture names to color codes.
 * Color values for linenoise:
 *   0 = default, 1 = red, 2 = green, 3 = yellow, 4 = blue,
 *   5 = magenta, 6 = cyan, 7 = white
 *   Add 8 for bold
 */
static char get_color_for_capture(const char *name, uint32_t len) {
    /* Keywords - bold magenta */
    if (len >= 7 && strncmp(name, "keyword", 7) == 0) {
        return 13; /* bold magenta */
    }
    ...
    /* Booleans and nil - bold yellow */
    if (len >= 7 && strncmp(name, "boolean", 7) == 0) {
        return 11; /* bold yellow */
    }
    ...
    /* Parameters - italic cyan (just cyan in linenoise) */
    ...
    /* Variables - default (unless special) */
    if (len >= 8 && strncmp(name, "variable", 8) == 0) {
        /* Check for variable.builtin like self */
        if (len > 8 && strncmp(name + 8, ".builtin", 8) == 0) {
            return 14; /* bold cyan */
        }
        return 0; /* default */
    }
